...
class _OrderProcessingScreenState extends ConsumerState<OrderProcessingScreen>
  ...
  Future<void> _processOrder() async {
    try {
      ...
      if (paymentMethod.toLowerCase() == 'cod' || paymentMethod.toLowerCase() == 'cash on delivery') {
        // COD orders - complete immediately
        ...
        // UPI/Online payment - initiate payment flow
        await _initiatePaymentFlow(order.id, paymentMethod, total);
      }
  ...
  /// Initiate payment flow for UPI/online payments
  Future<void> _initiatePaymentFlow(String orderId, String paymentMethod, double total) async {
    try {
      setState(() {
        _currentMessage = 'Preparing payment gateway...';
      });
      ...
      // TODO: NEXT PHASE - Replace with real backend integration
      // Connect to: POST /api/v1/payments/create-order-with-payment
      //
      // final upiService = UpiPaymentService(client: ref.read(httpClientProvider));
      // final orderRequest = OrderWithPaymentRequest(
      //   cartItems: widget.orderData['items'],
      ...
      final mockRazorpayOrder = RazorpayOrderResponse(
        ...
        key: 'rzp_test_key', // This should come from backend
        internalOrderId: orderId,
        upiIntentUrl: null, // Will be generated by Razorpay SDK
      ...
      debugPrint('OrderProcessingScreen: Payment initiation failed - $e');
      _showError('Failed to start payment process. Please try again.');
    }
  }
