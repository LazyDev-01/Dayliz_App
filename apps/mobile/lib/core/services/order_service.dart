...
class OrderService {
  ...
  Future<domain.Order> createOrder({
    ...
    try {
      ...
      final orderId = response['id'] ?? response;

      // Fetch the complete order with joined data
      ...
      debugPrint('OrderService: Complete order fetched: $completeOrder');

      // Convert response to Order entity
      return _mapDatabaseOrderToEntity(completeOrder);
  ...
  domain.Order _mapDatabaseOrderToEntity(Map<String, dynamic> data) {
    try {
      ...
      // Extract address - handle missing address data gracefully
      final addressData = data['addresses'] ?? {};
      final shippingAddress = Address(
        id: addressData['id'] ?? 'temp-address-${data['id']}',
        userId: addressData['user_id'] ?? data['user_id'] ?? '',
        addressLine1: addressData['address_line1'] ?? 'Address not loaded',
        addressLine2: addressData['address_line2'] ?? '',
        city: addressData['city'] ?? 'City not loaded',
        state: addressData['state'] ?? 'State not loaded',
        postalCode: addressData['postal_code'] ?? '000000',
        country: addressData['country'] ?? 'Country not loaded',
      ...
      // Extract payment method - handle missing payment data gracefully
      final paymentData = data['payment_methods'] ?? {};
      final paymentMethod = PaymentMethod(
        id: paymentData['id'] ?? 'temp-payment-${data['id']}',
        userId: paymentData['user_id'] ?? data['user_id'] ?? '',
      ...
      return domain.Order(
        id: data['id'] ?? '',
        userId: data['user_id'] ?? '',
        orderNumber: data['order_number'],  // Add order number mapping
        items: orderItems,
